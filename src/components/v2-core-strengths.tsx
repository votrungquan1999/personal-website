...
export function V2CoreStrengths() {
  ...
  const strengths = [
    ...
    },
    {
      ...
      expanded: "Accessibility isn't an afterthought — it's a core requirement. I build with Radix primitives and ARIA patterns from day one, ensuring every user can navigate efficiently. Performance and accessibility go hand in hand in my work.",
      example: "Partners Portal — Power users' productivity dropped 30% as constant keyboard-to-mouse switching broke their flow during data-heavy tasks. I built accessible UI using Radix primitives with ARIA patterns, achieving 100% keyboard navigation that enabled users to work 30% faster without breaking their mental flow.",
    ...
    },
    {
      ...
      expanded: "I design backend systems with performance and maintainability in mind. Multi-layer caching, query optimization, and event-driven architecture are tools I use to build scalable solutions that handle real-world traffic. I solve problems not just for customers, but also for systems and developers.",
      example: "Partners Portal — Developers were afraid to deploy changes or perform database migrations because tight coupling meant every change risked breaking unrelated functionality, requiring days of manual testing. I implemented event-driven architecture and event sourcing, decoupling services and enabling confident deployments. This reduced error resolution time by 60% and improved API response times by 70%.",
